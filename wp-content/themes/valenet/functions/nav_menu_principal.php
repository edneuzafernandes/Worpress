...
function menu_principal($menu_name){
  if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
      ...
      $menu_list = '<nav class="main-header__principal-menu">';
      $menu_list .= '<ul class="header-primary-menu__list">';
      $count = 0;
          ...
          if ( !$menu_item->menu_item_parent ) {
              ...
              if(!empty($menu_items[$count + 1]) && $menu_items[ $count + 1 ]->menu_item_parent == $parent_id ){//Checking has child
                  ...
                  if ($menuCorrente == $title) {
                    // echo $menuCorrente;
                    $menu_list .= '<li class=" active '.$ac.'">' ."\n";$menu_list .= '<a '.$target.' href="'.$link.'" class="main-header__principal-menu-link main-header__principal-menu-link--active '.$ac2. " " .$menu_item->classes[0] .'" data-menu="'.$menu_item->attr_title.'">'.$title.'</a>' ."\n";
                } elseif ($menu_item->object == $menuCorrente) {
                    $menu_list .= '<li class=" active '.$ac.'">' ."\n";$menu_list .= '<a href="'.$link.'" '.$target.' class="main-header__principal-menu-link main-header__principal-menu-link--active '.$ac2. " " .$menu_item->classes[0] .'" data-menu="'.$menu_item->attr_title.'">'.$title.'</a>' ."\n";
                 } else {
                     $menu_list .= '<li class="menuli'.$ac.'">' ."\n"; $menu_list .= '<a href="'.$link.'" '.$target.' class="main-header__principal-menu-link '.$ac2. " " .$menu_item->classes[0] .'" data-menu="'.$menu_item->attr_title.'">'.$title.'</a>' ."\n";
                 }
              }
