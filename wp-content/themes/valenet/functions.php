...
require ('functions/function_faq.php');
...
function font_awesome_icons(){ ?>
  <link href="https://use.fontawesome.com/5ea516cbb3.css"  rel="stylesheet">
    <style>
    .dashicons-noticia:before {
        content: "\f1ea" !important;
        font-family: 'FontAwesome' !important;
        font-size: 18px !important;
    }

    .dashicons-depoimentos:before {
        content: "\f27a" !important;
        font-family: 'FontAwesome' !important;
        font-size: 18px !important;
    }

    .dashicons-eventos:before {
        content: "\f073" !important;
        font-family: 'FontAwesome' !important;
        font-size: 18px !important;
    }

    .dashicons-cases:before {
        content: "\f2b5" !important;
        font-family: 'FontAwesome' !important;
        font-size: 18px !important;
    }

    .dashicons-solucoes:before {
        content: "\f005" !important;
        font-family: 'FontAwesome' !important;
        font-size: 18px !important;
    }

    .dashicons-vagasDisponiveis:before {
...
function my_acf_google_map_api( $api ){
  ...
  // $api['key'] = 'AIzaSyAMqS-kXVh4Qk7n9Om9KkbwpoEnUmK41i8';
	return $api;
}
...
function verificaCookie(){
  ...
  if(!isset($_COOKIE['cidade']) and !isset($_COOKIE['distrito'])){
    wp_redirect( home_url('splash') , 301);
    exit;
  }
}
...
// function registraScript() {
//   $version = date("ymd-Gis");
//   // registra o javascript
//   // wp_enqueue_script( 'dispara-click-lista-canais', get_template_directory_uri() .'/assets/js/scripts.min.js', array( 'jquery' ), $version, true );
//   // declara que a url do arquivo faz parte do (wp-admin/admin-ajax.php)
//   wp_localize_script( 'dispara-click-lista-canais', 'disparaGradeCanais', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
// }
// add_action( 'wp_enqueue_scripts', 'registraScript' );

add_action('wp_ajax_dispara_lista_canais', 'dispara_lista_canais');
add_action('wp_ajax_nopriv_dispara_lista_canais', 'dispara_lista_canais' );
function dispara_lista_canais(){
  $iniciaTeste = new produtosValeNet();
  $idCanais = $_POST['idCanais'];
  // echo $idCanais;
  echo $iniciaTeste->loopCanaisCombo( $idCanais );
}

add_action( 'wpcf7_init', 'custom_add_form_tag_produto' );

function custom_add_form_tag_produto() {
  wpcf7_add_form_tag( 'produto', 'custom_clock_form_tag_handler' ); // "clock" is the type of the form-tag
}

function custom_clock_form_tag_handler( $tag ) {
  return "<input type='hidden' name='produto' id='idProduto' class='idProduto' value=''>";
}